...
XGtest=as.matrix(XGclean(answer.test)) ############# test 데이터셑 넣기
dtrain = xgb.DMatrix(XGtrain[,-(1:2)], label = XGtrain[,2])
dtest = xgb.DMatrix(XGtest[,-(1:2)], label = XGtest[,2])
...
glm.pred.test=predict(test.glm, newdata=answer.test, type="response") ##위에서 만들어진 glm으로 test셑 넣어서 test 확률값 만들기 
